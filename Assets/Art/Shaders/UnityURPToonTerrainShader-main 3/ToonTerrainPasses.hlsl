...
void InitializeInputData(Varyings IN, half3 normalTS, out InputData inputData)
{
    ...
    inputData.viewDirectionWS = viewDirWS;
        ...
        half4 clipPos = TransformWorldToHClip(inputData.positionWS);
...
#endif
{
    ...
    //putting lights here to sample for the shadow hatches. hope it works

    InputData inputData;
    InitializeInputData(IN, normalTS, inputData);
    //SETUP_DEBUG_TEXTURE_DATA(inputData, IN.uvMainAndLM.xy, _BaseMap);

    // Prepare shadow coordinates and sample the main light with shadows
    float4 shadowCoord;
    
    #if defined(_MAIN_LIGHT_SHADOWS_SCREEN)
        float4 clipPos = TransformWorldToHClip(GetAbsolutePositionWS(inputData.positionWS));
        shadowCoord = ComputeScreenPos(clipPos);
    #else
        shadowCoord = TransformWorldToShadowCoord(inputData.positionWS);
    #endif

    Light mainLight = GetMainLight(shadowCoord);
    float shadowMask = 1.0 - saturate(mainLight.shadowAttenuation);

    half NdotL = saturate(dot(inputData.normalWS, mainLight.direction));

    // Sample shadow attenuation and clamp
    half shadowAttenuation = saturate(mainLight.shadowAttenuation.r);
    half distanceAttenuation = mainLight.distanceAttenuation;

    // Shadow attenuation: 1 = lit, 0 = full shadow


    // Optionally invert shadow attenuation if shadows appear inverted
    ...
    float2 tx = inputData.positionWS.zy * scale;
    float2 ty = inputData.positionWS.xz * scale;
    float2 tz = inputData.positionWS.xy * scale;

    float3 n = abs(normalize(inputData.normalWS));
    float3 weights = pow(n, 4.0);
    weights /= (weights.x + weights.y + weights.z);

    half4 cx = _VerticalTex.Sample(sampler_VerticalTex, tx) * weights.x;
    half4 cy = _VerticalTex.Sample(sampler_VerticalTex, ty) * weights.y;
    half4 cz = _VerticalTex.Sample(sampler_VerticalTex, tz) * weights.z;
    half4 baseColor = cx + cy + cz;

    half4 cx2 = _TriplanarTex.Sample(sampler_TriplanarTex, tx) * weights.x;
    half4 cy2 = _TriplanarTex.Sample(sampler_TriplanarTex, ty) * weights.y;
    half4 cz2 = _TriplanarTex.Sample(sampler_TriplanarTex, tz) * weights.z;
    half4 detailOverlay = cx2 + cy2 + cz2;

    float detailStrength = 1.0;
    half4 tri = lerp(baseColor, baseColor * detailOverlay, shadowMask * detailStrength);
...
/*
    ...
    half distanceAttenuation = mainLight.distanceAttenuation;
    float shadowMask = 1.0 - inputData.shadowAttenuation; */
    // Shadow attenuation: 1 = lit, 0 = full shadow
    ...
    half3 minAmbient = albedo * 0.05h;

    half3 toonLighting = directLight + max(ambientLight, minAmbient);
