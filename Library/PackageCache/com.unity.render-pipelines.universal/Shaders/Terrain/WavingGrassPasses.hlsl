...
{
    // You can add a simplified GBuffer version here if needed,
    // or just return dummy values if you're not using this path.
    ...
    half4 LitPassFragmentGrass(GrassVertexOutput input) : SV_Target
    {
        ...
        // Sample base texture only
        half4 texColor = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, input.uv);

        // Optional: fade out by vertex alpha (for distance culling)
        ...
        // Optional: apply vertex fog only
    #if defined(_FOG_FRAGMENT)
        texColor.rgb = MixFog(texColor.rgb, input.fogFactorAndVertexLight.x);
    #endif
